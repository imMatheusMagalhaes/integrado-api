...
class UniversityRepository {
  ...
  create = async (obj) => {
    ...
    const data = await this.model.insertMany(target_object, { lean: true })
    this._checkIfError({ data, obj, method: this.create.name })
    return data
  }
  find = async (obj) => {
    const { target_object, options } = obj
    const data_to_show = this.objIsEmpty(target_object) ? { name: 1, country: 1, "state-province": 1 } : {}
    const data = await this.model.find(target_object, data_to_show, options)
